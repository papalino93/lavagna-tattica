...
export default async function PartitaPage({
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-8">
      ...
      <h1 className="mt-2 text-2xl font-semibold text-[var(--ink)]">vs {match.opponent}</h1>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3">
        ...
        >
          <p className="font-medium text-[var(--ink)]">Convocati</p>
          <p className="mt-0.5 text-sm text-[var(--ink-dim)]">{calledUpCount ?? 0} selezionati</p>
        </Link>
        <Link
          href={`/partite/${id}/formazione`}
          className="rounded-xl border border-[var(--line)] bg-[var(--surface-raised)] p-4 transition-colors hover:border-[var(--brand-border)] hover:bg-[var(--brand-soft)]"
        >
          <p className="font-medium text-[var(--ink)]">Formazione</p>
          <p className="mt-0.5 text-sm text-[var(--ink-dim)]">{formation?.module ?? "Da impostare"}</p>
        </Link>
      </div>
